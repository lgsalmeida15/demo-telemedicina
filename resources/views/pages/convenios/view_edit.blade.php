            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header card-header-primary">
                        <h4 class="card-title">Atualizar Serviço</h4>
                        <p class="card-category">Campos marcados com * são obrigatórios</p>
                    </div>

                    <div class="card-body">
                        <form action="{{ route('convenio.update', $convenio->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="partner_id" id="partner_select" class="form-control" required>
                                            ...
                                            @foreach($partners as $partner)
                                                <option value="{{ $partner->id }}" {{ $convenio->partner_id == $partner->id ? 'selected' : '' }}>
                                                    {{ $partner->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select name="convenio_categoria_id" id="convenio_categoria_id" class="form-control" required>
                                            ...
                                            @foreach($categorias as $categoria)
                                                <option value="{{ $categoria->id }}" {{ $convenio->convenio_categoria_id == $categoria->id ? 'selected' : '' }}>
                                                    {{ $categoria->nome }}
                                                </option>
                                            @endforeach
                                        </select>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <select name="convenio_type_id" id="convenio_type_id" class="form-control" required>
                                            <option value="" disabled>Selecione ou crie um tipo</option>
                                            @foreach($types as $type)
                                                <option value="{{ $type->id }}" {{ $convenio->convenio_type_id == $type->id ? 'selected' : '' }}>
                                                    {{ $type->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="nome_convenio" id="nome_convenio" class="form-control" value="{{ $convenio->nome_convenio }}" maxlength="255" required>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                <div class="form-group">
                                    ...
                                    <textarea name="descricao" id="descricao" rows="3" class="form-control">{{ $convenio->descricao }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <input type="number" step="0.01" name="desconto_percentual" id="desconto_percentual" class="form-control" value="{{ $convenio->desconto_percentual }}">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <h5 for="data_inicio" class="form-label">Data de Início<span style="color: red"> *</span></h5>
                                        <input type="date" name="data_inicio" id="data_inicio" class="form-control" value="{{ $convenio->data_inicio }}" required>
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <h5 for="data_fim" class="form-label">Data de Fim</h5>
                                        <input type="date" name="data_fim" id="data_fim" class="form-control" value="{{ $convenio->data_fim }}">
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="row mb-3">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="contato" id="contato" class="form-control" maxlength="255" value="{{ $convenio->contato }}">
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="status" id="status" class="form-control" required>
                                        @foreach(["Ativo", "Inativo"] as $statusOption)
                                            <option value="{{ $statusOption }}" {{ $convenio->status == $statusOption ? "selected" : "" }}>{{ $statusOption }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="btn-flutante d-flex justify-content-between gap-3">
                                <button type="submit" class="btn btn-primary btn-lg w-50">ATUALIZAR SERVIÇO</button>
