                    ...
                    <form id="filtroForm" method="GET" action="{{ route('financial.index') }}" class="row align-items-end">
                        ...
                        <div class="col-md-3 d-flex flex-column mb-2">
                            ...
                            <input type="date" id="filtroDataFim" name="data_fim"
                                value="{{ request('data_fim') }}" class="form-control">
                        </div>
                        ...
                        <div class="col-md-2 mb-2">
                            <label class="small mb-1 font-weight-bold">Caixa</label>
                            <select name="caixa_id" id="caixa_id" class="form-control">
                                <option value="">Todos</option>
                                @foreach ($caixas as $caixa)
                                <option value="{{ $caixa->id }}" {{ (string) $caixa->id === ($filtro['caixa_id'] ?? '') ? 'selected' : '' }}>
                                    {{$caixa->descricao}}
                                </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-2 text-right">
                            ...
                            <button type="button" id="btnClear" class="btn btn-danger btn-block">
                                <i class="material-icons mr-1" style="font-size:18px;">clear</i> Limpar
                            </button>
                        </div>
                    </form>
                </div>
                ...
                <div class="col-lg-4 col-md-6">
                    <div class="card card-stats shadow-sm">
                        ...
                        <div class="card-body text-center">
                            ...
                            <h4 class="card-title text-success" >R$ {{$entradas}}</h4>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-6">
                    <div class="card card-stats shadow-sm">
                        ...
                        <div class="card-body text-center">
                            ...
                            <h4 class="card-title text-danger" >R$ {{$saidas}}</h4>
                        </div>
                    </div>
                </div>
                <div class="col-lg-4 col-md-12">
                    <div class="card card-stats shadow-sm">
                        ...
                        <div class="card-body text-center">
                            ...
                            <h4 class="card-title" >R$ {{$saldo}}</h4>
                        </div>
                    </div>
                </div>
            </div>
...
{{-- Script JS para o botão de limpar --}}
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (clearButton) {
            clearButton.addEventListener('click', function() {
                // Seleciona todos os inputs do formulário de filtro
                const form = document.getElementById('filtroForm');
                const inputs = form.querySelectorAll('input, select');
                
                // Reseta o valor de cada input
                inputs.forEach(input => {
                    if (input.type === 'date' || input.type === 'text' || input.tagName === 'SELECT') {
                        input.value = ''; // Limpa o valor do input/select
                    }
                });

                // Dispara a submissão do formulário para recarregar a página sem filtros
                form.submit();
            });
        }
    });
